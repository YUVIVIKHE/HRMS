...
// Manager's team
$myTeam       = (int)$db->prepare("SELECT COUNT(*) FROM users WHERE manager_id=? AND role='employee'")->execute([$uid]) ? $db->query("SELECT COUNT(*) FROM users WHERE manager_id=$uid AND role='employee'")->fetchColumn() : 0;
$pendingLvs   = 5; // Static placeholder
$todayPresent = 12; // Static placeholder
$openTasks    = 8; // Static placeholder

// Team leave requests
$teamLeaves = []; // Static placeholder

// Team members
$teamMembers = $db->query(
    "SELECT id, name, email, status FROM users WHERE manager_id=$uid AND role='employee' LIMIT 8"
)->fetchAll();
?><!DOCTYPE html>
<html lang="en">
<head>
...
<style>
:root{--bg:#0a0f1e;--surface:#111827;--surface2:#1a2234;--border:rgba(255,255,255,.07);--accent:#1d4ed8;--accent2:#3b82f6;--red:#ef4444;--green:#22c55e;--yellow:#eab308;--text:#e2e8f0;--muted:#8896aa;--sidebar:240px;--r:14px}
...
.s-logo .lb{width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,#1d4ed8,#3b82f6);display:flex;align-items:center;justify-content:center;font-size:1.3rem}
...
nav a:hover,nav a.active{background:rgba(59,130,246,.1);color:var(--text);border-left-color:var(--accent2)}
...
.av{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#1d4ed8,#3b82f6);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.9rem}
...
.ab:hover{background:rgba(59,130,246,.15);transform:translateY(-2px)} .ab .ic{font-size:1.5rem}
@media(max-width:1100px){.stats{grid-template-columns:repeat(2,1fr)}.panels{grid-template-columns:1fr}}
</style>
</head>
<body>
<aside class="sidebar">
  <div class="s-logo"><div class="lb">🎯</div><div><strong>HRMS</strong><small>Manager Panel</small></div></div>
  <nav>
    ...
    <div class="ns">Team</div>
    <a href="my_team.php"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg><span>My Team</span></a>
    ...
    <a href="tasks.php"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg><span>Tasks</span></a>
...
<div class="main">
  ...
  <div class="content">
    <div class="ph">
      ...
      <p><?= date('l, F j, Y') ?> — Your team overview for today.</p>
    </div>

    <div class="stats">
      <div class="sc"><div class="si" style="background:rgba(59,130,246,.15);color:#60a5fa">👥</div><div><div class="val"><?= $myTeam ?></div><div class="lbl">My Team Size</div><div class="chg" style="color:#4ade80">↑ Direct reports</div></div></div>
      <div class="sc"><div class="si" style="background:rgba(234,179,8,.15);color:#fbbf24">📋</div><div><div class="val"><?= $pendingLvs ?></div><div class="lbl">Pending Leaves</div><div class="chg" style="color:#fbbf24">⚠ Action needed</div></div></div>
      <div class="sc"><div class="si" style="background:rgba(34,197,94,.15);color:#4ade80">✅</div><div><div class="val"><?= $todayPresent ?></div><div class="lbl">Present Today</div><div class="chg" style="color:#4ade80">↑ Checked in</div></div></div>
      <div class="sc"><div class="si" style="background:rgba(168,85,247,.15);color:#c084fc">📌</div><div><div class="val"><?= $openTasks ?></div><div class="lbl">Open Tasks</div><div class="chg" style="color:#c084fc">↑ In progress</div></div></div>
    </div>

    <div class="panels">
      <div class="panel">
        <div class="ph2"><h2>Team Leave Requests</h2><a href="leaves.php">View all →</a></div>
        <table>
          <thead><tr><th>Employee</th><th>Type</th><th>From</th><th>To</th><th>Status</th></tr></thead>
          <tbody>
          <?php if(empty($teamLeaves)): ?>
            <tr><td colspan="5" style="text-align:center;color:var(--muted);padding:28px">No leave requests</td></tr>
          <?php else: foreach($teamLeaves as $l): ?>
            <tr>
              <td style="font-weight:600"><?= htmlspecialchars($l['name']) ?></td>
              <td style="font-size:.8rem;color:var(--muted)"><?= htmlspecialchars($l['leave_type']) ?></td>
              <td style="font-size:.79rem"><?= date('M d',strtotime($l['start_date'])) ?></td>
              <td style="font-size:.79rem"><?= date('M d',strtotime($l['end_date'])) ?></td>
      ...
      <div style="display:flex;flex-direction:column;gap:18px">
        <div class="panel">
          ...
          <div class="ag">
            <a href="leaves.php?filter=pending" class="ab"><span class="ic">✅</span><span>Approve Leaves</span></a>
            <a href="tasks.php?action=new" class="ab"><span class="ic">📌</span><span>Assign Task</span></a>
            <a href="attendance.php" class="ab"><span class="ic">📊</span><span>Attendance</span></a>
            <a href="my_team.php" class="ab"><span class="ic">👥</span><span>View Team</span></a>
          </div>
        </div>
        <div class="panel">
          <div class="ph2"><h2>My Team</h2><a href="my_team.php">View all →</a></div>
          <table>
            <thead><tr><th>Name</th><th>Status</th></tr></thead>
            <tbody>
            <?php if(empty($teamMembers)): ?>
              <tr><td colspan="2" style="text-align:center;color:var(--muted);padding:22px">No team members</td></tr>
            <?php else: foreach($teamMembers as $m): ?>
              <tr>
                <td><div style="font-weight:600;font-size:.83rem"><?= htmlspecialchars($m['name']) ?></div><div style="font-size:.72rem;color:var(--muted)"><?= htmlspecialchars($m['email']) ?></div></td>
