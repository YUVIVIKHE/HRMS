...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action']??'') === 'submit_acl') {
    ...
    elseif (!$reason) { $errorMsg = 'Provide a reason.'; }
        ...
        if ($chk->fetch()) { $errorMsg = 'Already requested for this date.'; }
            ...
            $_SESSION['flash_success'] = "ACL request submitted.";
...
<div class="main-content">
  ...
  <div class="page-body">
      ...
      <div class="card-body">
        <form method="POST">
          ...
          <div class="form-grid" style="max-width:500px;">
            <div class="form-group"><label>Work Date <span class="req">*</span></label><input type="date" name="work_date" class="form-control" required></div>
            <div class="form-group"><label>Hours</label><input type="number" name="hours" class="form-control" value="9" min="1" max="12" step="0.5"></div>
          </div>
          <div class="form-group" style="margin-top:12px;max-width:500px;">
            <label>Reason <span class="req">*</span></label>
            <textarea name="reason" class="form-control" rows="2" placeholder="Reason for working on holiday…" required></textarea>
          </div>
    ...
    <div class="table-wrap">
      <div class="table-toolbar"><h2>My ACL Requests</h2></div>
      <table>
        <thead><tr><th>Date</th><th>Reason</th><th style="text-align:center;">Hours</th><th>Status</th><th>Note</th></tr></thead>
        <tbody>
          <?php if(empty($requests)): ?><tr class="empty-row"><td colspan="5">No requests yet.</td></tr>
